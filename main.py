			...
			if admin_s in admin_str:
				...
				if bnd_id != 5376177894:
					try:
						...
						new_time = now_time.date + " " + now_time.hour + ":" + now_time.minute + ":" + now_time.second
						if bnd_id not in MyBot.ban_list[chat_id]:
								
							bot.ban_chat_member(chat_id, bnd_id)
							MyBot.ban_list[chat_id][bnd_id] = f"""
									
Banned Time and Date: {new_time}
...
Succesfully Banned {mention_text}""",parse_mode=ParseMode.HTML,)
						...
						update.message.reply_text("I Don't Ban Admins 💀")
		...
		if "/unban" == msg:
			if admin_s in admin_str:
				try:
					...
					if bnd_id != 5376177894:
						if nm_id in MyBot.ban_list[chat_id]:
